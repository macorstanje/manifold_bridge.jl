...
function κ(t, y , z, M::Manifolds.EmbeddedTorus, B::AbstractBasis)
    ...
    out = sum([ exp(-(θ1 - θ2 - 2*pi*k)^2/(2*t/M.r^2) - (ϕ1 - ϕ2 - 2*pi*l)^2/(2*t/M.R^2)) 
                for k in -truncation:truncation, l in -truncation:truncation])
    return out/(2*pi*t/(M.R*M.r))
end
...
function κ(t, y , z, trunc, M::Manifolds.EmbeddedTorus, B::AbstractBasis)
    θ1 = y[1] + B.i[1] ; ϕ1 = y[2] + B.i[2]
    θ2 = z[1] + B.i[1] ; ϕ2 = z[2] + B.i[2]
    out = sum([ exp(-(θ1 - θ2 - 2*π*k)^2/(4*t/M.r^2) - (ϕ1 - ϕ2 - 2*pi*l)^2/(4*t/M.R^2)) 
                for k in -trunc:trunc, l in -trunc:trunc])
    return out/(4*pi*t/(M.R*M.r))
end

# Guiding function in local coordinates. 
function g_param(M::Manifolds.EmbeddedTorus, B::AbstractBasis, t, a, trunc, obs::observation) 
    return κ(obs.t - t , a, get_parameters(M, B.A, B.i, obs.u[1] ), trunc, M, B)
end


# Riemannian gradient of log g, in local coordinates
function ∇logg(M::Manifolds.EmbeddedTorus, B::AbstractBasis, t, a, trunc, obs::observation)
